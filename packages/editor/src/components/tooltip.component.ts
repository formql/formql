...
@Component({
  ...
  template: `
    <div
      [ngClass]="[type == ContainerType.Component ? 'fql-editor-tooltip-center' : 'fql-editor-tooltip-section-center']"
    >
...
export class TooltipComponent {
  ...
  remove() {
    const result = confirm(
      `Are you sure you want to remove this ${this.type === ContainerType.Component ? 'Component' : 'Section'}?`
    );
    if (result)
      if (this.type === ContainerType.Component)
        this.internalEventHandlerService.send(InternalEventType.RemoveComponent, this.object);
      else this.internalEventHandlerService.send(InternalEventType.RemoveSection, this.object);
  }
}
